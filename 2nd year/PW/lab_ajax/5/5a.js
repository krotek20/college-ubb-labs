...
function addClickEvents() {
    document.querySelectorAll(".directory").forEach((item) => {
...
function openFile(fileUrl) {
    ...
    let url = "5Openfile.php?file=" + fileUrl;
    console.log(url);
...
function displayTree() {
    ...
    if (currentFiles.includes(content)) {
        currentFiles = currentFiles.filter(function (el) {
            return el !== content;
        })
    } else {
...
function getTree() {
    ...
    let url = "5Tree.php?visible=" + getVisibleFoldersUrl();
    console.log(url);
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
            document.getElementById("container").innerHTML = this.responseText;
